...
/// CR 616.1: the commute class of a `ProduceMana` replacement's `mana_modification`.
/// Multipliers (Nyxbloom Ancient ×3, Mana Reflection ×2, Zhur-Taa Druid-style ×2)
/// scale the produced count, so two of them yield the same product regardless of
/// application order ((c·3)·2 == (c·2)·3) — they commute and must auto-resolve
/// rather than force a degenerate CR 616.1 ordering prompt. A `ReplaceWith` type
/// override is last-write-wins, so two *different* overrides are order-material
/// and remain a genuine choice.
